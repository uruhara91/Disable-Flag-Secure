...
constexpr char kTargetsPath[] =
        "/data/adb/modules/zygisk_secure_capture/config/targets.txt";
constexpr char kExcludesPath[] =
        "/data/adb/modules/zygisk_secure_capture/config/exclude.txt";
...
uint64_t ComputeChecksum(const ConfigSnapshot& snapshot) noexcept {
    constexpr size_t checksum_offset =
            offsetof(ConfigSnapshot, header) +
            offsetof(ConfigHeader, checksum);
    constexpr size_t after_checksum =
            checksum_offset + sizeof(uint64_t);

    uint64_t hash = common::HashBytes(&snapshot, checksum_offset);
    const auto* bytes = reinterpret_cast<const uint8_t*>(&snapshot);
    return common::HashBytes(
            bytes + after_checksum,
            sizeof(ConfigSnapshot) - after_checksum,
            hash);
}

void NormalizeUnsupportedState(ConfigSnapshot* snapshot) noexcept {
    snapshot->header.flags &= kCompiledFeatureFlags;
    if ((kCompiledFeatureFlags & kTargetScopedFeatureFlags) == 0) {
        snapshot->header.target_count = 0;
        snapshot->header.exclude_count = 0;
        memset(snapshot->targets, 0, sizeof(snapshot->targets));
        memset(snapshot->excludes, 0, sizeof(snapshot->excludes));
    }
}

void Finalize(ConfigSnapshot* snapshot) noexcept {
    NormalizeUnsupportedState(snapshot);
...
void SetFlag(ConfigSnapshot* snapshot, FeatureFlag flag,
             bool enabled) noexcept {
    const uint32_t bit = static_cast<uint32_t>(flag);
    if ((bit & kCompiledFeatureFlags) == 0) return;
    if (enabled) {
        snapshot->header.flags |= bit;
    } else {
        snapshot->header.flags &= ~bit;
    }
}
...
void InsertHash(uint64_t* values, uint32_t* count,
                uint64_t hash) noexcept {
    if (values == nullptr || count == nullptr || hash == 0 ||
        *count >= kMaxPackageHashes) {
        return;
    }
...
void ReadPackageFile(const char* path, uint64_t* values,
    ...
    while (fgets(line, sizeof(line), file) != nullptr) {
        ...
        if (IsValidPackage(text, length)) {
            InsertHash(values, count,
                       common::HashAscii(text, length));
        }
    }
    fclose(file);
}
    ...
    while (fgets(line, sizeof(line), file) != nullptr) {
        ...
        *separator = '\0';
        ...
        for (const Option& option : options) {
            if (strcmp(key, option.name) != 0) continue;
            SetFlag(snapshot, option.flag,
                    ParseBoolean(value,
                                 HasFlag(*snapshot, option.flag)));
            break;
        }
    }
    fclose(file);
...
bool BinaryContains(const uint64_t* values, uint32_t count,
                    uint64_t hash) noexcept {
    uint32_t left = 0;
    uint32_t right = count;
    while (left < right) {
        const uint32_t middle = left + (right - left) / 2;
        if (values[middle] < hash) {
            left = middle + 1;
        } else {
            right = middle;
        }
...
bool IsStrictlySorted(const uint64_t* values,
                      uint32_t count) noexcept {
...
bool UnusedEntriesAreZero(const uint64_t* values,
                          uint32_t count) noexcept {
    for (size_t i = count; i < kMaxPackageHashes; ++i) {
...
void LoadSnapshot() noexcept {
    ...
    if ((kCompiledFeatureFlags & kTargetScopedFeatureFlags) != 0) {
        ReadPackageFile(
                kTargetsPath,
                g_companion_snapshot.targets,
                &g_companion_snapshot.header.target_count);
        ReadPackageFile(
                kExcludesPath,
                g_companion_snapshot.excludes,
                &g_companion_snapshot.header.exclude_count);
    }
...
bool ValidateSnapshot(const ConfigSnapshot& snapshot) noexcept {
    return snapshot.header.magic == kConfigMagic &&
           snapshot.header.version == kConfigVersion &&
           snapshot.header.header_size == sizeof(ConfigHeader) &&
           snapshot.header.struct_size == sizeof(ConfigSnapshot) &&
           snapshot.header.target_count <= kMaxPackageHashes &&
           snapshot.header.exclude_count <= kMaxPackageHashes &&
           (snapshot.header.flags & ~kCompiledFeatureFlags) == 0 &&
           snapshot.header.reserved == 0 &&
           IsStrictlySorted(snapshot.targets,
                            snapshot.header.target_count) &&
           IsStrictlySorted(snapshot.excludes,
                            snapshot.header.exclude_count) &&
           UnusedEntriesAreZero(snapshot.targets,
                                snapshot.header.target_count) &&
           UnusedEntriesAreZero(snapshot.excludes,
                                snapshot.header.exclude_count) &&
           snapshot.header.checksum == ComputeChecksum(snapshot);
}

bool HasFlag(const ConfigSnapshot& snapshot,
             FeatureFlag flag) noexcept {
    return (snapshot.header.flags &
            static_cast<uint32_t>(flag)) != 0;
}

bool ContainsTarget(const ConfigSnapshot& snapshot,
                    uint64_t exact, uint64_t base) noexcept {
    return BinaryContains(snapshot.targets,
                          snapshot.header.target_count,
                          exact) ||
           (base != exact &&
            BinaryContains(snapshot.targets,
                           snapshot.header.target_count,
                           base));
}

bool ContainsExclude(const ConfigSnapshot& snapshot,
                     uint64_t exact, uint64_t base) noexcept {
    return BinaryContains(snapshot.excludes,
                          snapshot.header.exclude_count,
                          exact) ||
           (base != exact &&
            BinaryContains(snapshot.excludes,
                           snapshot.header.exclude_count,
                           base));
}

bool LoadFromCompanion(zygisk::Api* api,
                       ConfigSnapshot* output) noexcept {
...
void CompanionHandler(int client) noexcept {
    ...
    if (pthread_once(&g_snapshot_once, LoadSnapshot) != 0) {
        ...
        (void)common::WriteFull(
                client, &disabled, sizeof(disabled));
        return;
    }
    (void)common::WriteFull(
            client, &g_companion_snapshot,
            sizeof(g_companion_snapshot));
}
